        ...
        <div class="justify-content-center">
            <div class="row">
                <div class="col-md-12 mb-5">
                    <span style="font-size: 25px;font-weight: 600">Equipe que faço parte</span>
                </div>
            ...
            <table id="example" class="display cell-border" style="width:100%">
                ...
                @foreach($equipes as $equipe)
                    <tr>
                        <td width="40%">{{$equipe->nome}}</td>
                        <td width="40%">{{ str_replace($grupo, "", $equipe->equipe) }}</td>
                        <td style="text-align: center">
                            <button class="permissao btn btn-success" data-id="{{$equipe->id}}" data-equipe="{{$equipe->equipe}}">Permissões</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    <div class="popup">
        <div class="card ">
            ...
            <div class="card-body">
                ...
                <form action="{{route("equipe.remover")}}" id="frmRemover" method="post" onsubmit="return confirmUpdate(event);">
                    @csrf
                    <input type="hidden" name="id" id="idRemover">
                    <input type="hidden" name="group" id="group" value="">
                    <input type="hidden" name="user"  id="user"  value="">
                </form>
                <form action="{{route('equipe.adicionar')}}" method="post" onsubmit="return confirmUpdate(event);">
                    @csrf
      ...
      function confirmUpdate(event) {
        ...
        $(document).ready(function() {
            ...
            $('.permissao').click(function () {
                ...
                $.ajax({
                    url : '{{route('equipe.getAcesso')}}',
                    method : "post",
                    data : {'id' : id, 'equipe' : equipe},
                    success : function (result) {
                        $('#idAcesso').val(id);
                        $('#permissao-email').html(equipe.replace("{{ $grupo }}", ''));
                        $('#lista').html(result);
